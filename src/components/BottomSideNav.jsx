...
export default function BottomSideNav({ isOpen }) {
  return (
    <div className={`hidden z-40 bg-black h-auto shadow-md pb-16 lg:block transition-transform hover:overflow-y-scroll scrollbar lg:w-60 transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} items-center justify-around lg:justify-normal fixed bottom-0 lg:top-16 lg:left-0 flex lg:flex-col lg:h-full px-4 shadow-lg`}>
      ...
      <div>
        ...
        <Link href={"/"} className='dark:hover:bg-zinc-800 hover:bg-gray-200 text-black py-3 flex flex-col lg:flex-row justify-start items-center rounded-md px-2 w-full text-sm dark:text-white space-x-4'><FaRegNewspaper size={18} className='text-black dark:text-white' /><span className='text-sm font-semibold'>News</span></Link>
